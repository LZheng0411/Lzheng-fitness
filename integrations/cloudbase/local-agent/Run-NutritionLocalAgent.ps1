[CmdletBinding()]
param([switch]$Once,[switch]$Watch,[string]$QueueFile,[string]$ConfigPath,[string]$JobId)
...
function Invoke-AgentCycle {
  ...
  $jobs=@(Read-JsonFile $QueueFile @());$job=@($jobs|Where-Object {$_.status -in @('queued','pending_agent') -and ((-not $JobId) -or $_.id -eq $JobId)}|Select-Object -First 1)
  ...
  finally{if(Test-Path -LiteralPath $temp){$resolvedTemp=[IO.Path]::GetFullPath($temp);$allowedTemp=[IO.Path]::GetFullPath([IO.Path]::GetTempPath());if($resolvedTemp.StartsWith($allowedTemp,[StringComparison]::OrdinalIgnoreCase)){try{Remove-Item -LiteralPath $resolvedTemp -Recurse -Force}catch{Write-Warning 'Temporary artifact cleanup incomplete'}}}}
}
function Invoke-SerializedAgentCycle([string]$RunMode){
  if((-not $QueueFile) -and $ConfigPath){$queueConfig=Read-JsonFile $ConfigPath $null;if($queueConfig.queue_file){$script:QueueFile=[string]$queueConfig.queue_file}}
