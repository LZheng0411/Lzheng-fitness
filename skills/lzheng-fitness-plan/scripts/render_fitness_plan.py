...
def render(plan: dict[str, Any], image_path: Path | None = None) -> str:
    ...
    days_html = "".join(render_day(day) for day in plan.get("training_days", []))
    return f"""<!doctype html>
<html lang="zh-CN" data-plan-id="{esc(meta.get('plan_id'))}" data-snapshot-id="{esc(snapshot.get('snapshot_id'))}">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>{esc(meta.get('title'))}</title>
<style>
:root{{--ink:#17221e;--muted:#66736d;--line:#dfe5e1;--soft:#f4f6f4;--green:#174f3d;--green2:#e7f0ec;--warn:#9a5b1f;--warnbg:#fff5e8;--white:#fff}}
*{{box-sizing:border-box}}html,body{{max-width:100%;overflow-x:hidden}}html{{scroll-behavior:smooth}}body{{margin:0;background:#eef1ef;color:var(--ink);font:15px/1.65 system-ui,-apple-system,"Segoe UI","Microsoft YaHei",sans-serif}}
...
.sticky{{position:sticky;top:0;z-index:10;background:#fffffff2;border-bottom:1px solid var(--line);backdrop-filter:blur(10px);overflow:auto;white-space:nowrap;padding:10px 44px}}.sticky a{{text-decoration:none;margin-right:20px;font-size:13px;color:var(--muted)}}
.section{{padding:48px 0;border-bottom:1px solid var(--line);scroll-margin-top:58px}}.section-title{{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-bottom:18px}}.eyebrow{{display:block;color:var(--green);font-size:12px;font-weight:750;letter-spacing:.08em;text-transform:uppercase}}.duration{{font-weight:700;color:var(--green)}}
...
.warmup{{margin:18px 0;padding:18px 20px;border:1px solid var(--line);border-radius:14px;background:var(--green2)}}.warmup ol{{margin:8px 0 0;padding-left:20px}}
...
@media(max-width:820px){{.wrap{{padding:0 20px}}.hero{{padding:40px 20px 28px;min-height:auto}}.hero-art{{display:none}}.sticky{{padding:10px 20px}}.quick,.detail-grid,.rule-grid{{grid-template-columns:1fr}}.schedule{{display:flex;overflow:auto}}.schedule-card{{min-width:140px}}.dose-grid{{grid-template-columns:1fr 1fr}}.section{{padding:34px 0}}.meta .snapshot-pill{{flex-basis:100%}}}}
...
<nav class="sticky"><a href="#start">快速开始</a><a href="#week">本周安排</a><a href="#stages">动作分层</a><a href="#coverage">覆盖检查</a><a href="#progression">渐进</a><a href="#fallback">降级接回</a><a href="#sources">依据</a></nav>
<div class="wrap">
