...
并在 `tracking_targets` 写入目标专属的工作台追踪项：增肌包含训练完成、重点肌群计划组数与双重渐进记录；减脂包含训练完成、体重趋势、日均步数与有氧时长；力量包含训练完成、关键动作实际表现与周期判定。每一项要写明数据来源、状态和下一步记录动作；未知基线只能写 `needs_baseline`。
