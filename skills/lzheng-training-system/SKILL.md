...
当用户刚完成安装、刚下载本套件，或问“接下来怎么用”时，不要要求用户先阅读 README，也不要只罗列命令。直接用自然语言告诉用户：

1. 选一个**新的空文件夹**作为系统根目录；
2. 用户只需说：`使用 $lzheng-training-system 帮我在 "<空文件夹路径>" 从零搭建我的 AI 健身系统。`；
3. 你随后负责初始化工作台、完成最少建档问诊、生成第一版计划，并解释示例页面不是正式处方；
4. 正式使用后，用户只需每次告诉你“今天练了什么”和主观体感；你负责复盘、写入记录、刷新工作台。

若当前聊天没有识别 `$lzheng-training-system`，说明安装后的 Skill 尚未被当前会话加载；提示用户新开一个聊天，然后重复上述固定句。不得把“看 README”当作继续使用的前置条件。
