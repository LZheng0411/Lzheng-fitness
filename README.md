...
验证会检查七个 Skill 的元数据、链接、隐私残留和脚本语法，另外核验六个专家模块的来源边界、知识文件、路由样例与安全分流；随后渲染既有 HTML 输出、校验工作台资源，并在临时空目录中完成整套安装、初始化与诊断。工作台还包含周切换回归闸门：进入新周前必须先同步当前计划排程，旧周日期、混合 Wn、漏掉今日处方、训练频率不符或“自重”被历史重量覆盖都会阻止刷新。
