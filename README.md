...
验证会检查七个 Skill 的元数据、链接、隐私残留和脚本语法，另外核验六个专家模块的来源边界、知识文件、路由样例与安全分流；随后渲染 HTML，校验工作台的图片、视频、海报、内置文档和计划入口，并在临时目录中完成初始化、改名移动、旧配置迁移、无 Obsidian 发布和故障拦截。固定盘符、移动后失效、资源漏复制、旧周日期、混合 Wn、漏掉今日处方、训练频率不符或“自重”被历史重量覆盖都会阻止验证。
