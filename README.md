...
可独立下载、离线运行的个人健身 Agent Skills。v3.3.0 候选新增视频学习、独立知识库、三餐规划和一句话升级，支持本机训练完成、训练日历与纠错恢复、餐食和照片保存、手动营养候选与确认入账。AI 计划与复盘需要支持 Skill 的 AI 助手；网页不自带照片识别模型。
...
- 对 Agent 说“学习我收藏里关于某主题的视频”，或直接给视频链接。支持不同主题，先确定范围，再分批准备来源、保存进度、失败重试；机器转写不等于用户学会。
- 知识库支持作者、专题、来源视频、表格和本机学习反馈。默认空白，可选择导入[谭成义饮食学习示例](docs/KNOWLEDGE-EXAMPLE-REVIEW.md)，示例不含使用者个人资料，也不会激活饮食计划。
- 手机/桌面常驻饮食与知识入口、自然周日历、本机改期与顺延、三餐目标版本和菜单估算。顺延保留完整处方，未排期不记为休息或完成。
...
主体包含 **10 个 MIT 自有 Skill**。另附原版 `dbs-learning` 可选交互学习组件，使用 **CC BY-NC 4.0**，不在主体 `--all` 内默认安装；仓颉使用 MIT，按需安装官方版本。请阅读[第三方许可](THIRD-PARTY-NOTICES.md)。非商业开源分享不意味着所有组件都可商用。
