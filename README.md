...
可独立下载、离线运行的个人健身 Agent Skills。v3.1.0 把训练计划、逐组记录、训练档案、有氧、营养契约、餐食确认与复盘组合为一个可迁移的本地闭环。
...
工作台默认完全本地运行：训练逐组记录、有氧和餐食候选保存到当前浏览器；没有登录、没有后台轮询、没有自动模型调用。餐食照片仅作为用户本地选择的候选，不会被公开包或默认页面上传。饭前估算、饭后校正候选、用户确认入账严格分开。
